...
class _FollowingScreenState extends State<FollowingScreen> {
  ...
  Widget build(BuildContext context) {
    _usersFollowing =  DatabaseService.displayFollowings(widget.userId
        );
      ...
      body: FutureBuilder(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.data.documents.length == 0) {
            print("length 0");
            return Center(child: Text("Currently no Followings", style: TextStyle(color: grey, fontSize: smallSizeText),));
          }
